<div class="row">
    <div class="col-12 d-flex flex-row align-items-center mb-4">
        ...
        <div class="ms-3 d-flex align-items-center">
            <button class="px-3 py-2 me-2"
                style="background-color: #ff000086; border: 1px solid red; border-radius: 5px;"></button> Prazo vencido
        </div>
    </div>
    <hr>
    <div class="col-12">
        <div class="row">
                ...
                <div class="row">
                    <div class="accordion row">
                        @foreach ($demandas as $demanda)
                            <div class="accordion-item col-12 mb-3">
                                <h2 class="accordion-header" id="demanda{{ $demanda->id }}">
                                    <button
                                        class="accordion-button @if ($demanda->estimativa < date('Y-m-d') && !$demanda->finalizada) vencida @endif collapsed"
                                        type="button" data-bs-toggle="collapse"
                                        data-bs-target="#collapseDemanda{{ $demanda->id }}" aria-expanded="true"
                                        aria-controls="collapseDemanda{{ $demanda->id }}"><span
                                            class="badge badge-soft-secondary font-size-12 me-2  @if ($demanda->estimativa < date('Y-m-d') && !$demanda->finalizada) text-white @endif">
                                            {{ date('d/m', strtotime($demanda->estimativa)) }} </span>
                                        {{ $demanda->titulo }}
                                        @if ($demanda->finalizada)
                                            <i class="fas fa-check-circle ms-4 fa-lg" style="color: green;"></i>
                                        @else
                                            <i class="fas fa-clock ms-4 fa-lg" style="color: orange;"></i>
                                            @endif @if ($demanda->urgente)
                                                <i class="fas fa-exclamation-circle fa-lg mx-3"
                                    ...
                                    <div class="accordion-body">
                                        <table class="table" style="width: 100%;">
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($demanda->finalizada)
                                                            {{ date('d/m/Y', strtotime($demanda->entrega)) }}
                                                        @else
                                                            A ser feita
                                                        @endif
                                                    </td>
                                                ...
                                                <tr>
                                                    <td class="text-center" colspan="4">
                                                        @if ($demanda->arquivo)
                                                            <a href="{{ asset($demanda->arquivo) }}" target="_blank">
                                        ...
                                        <div class="row">
                                            <div class="col-12 text-center">
                                                @if ($demanda->setor != 3)
                                                    <i class="fas fa-edit fa-lg cpointer"
                                                        wire:click='editar("{{ $demanda->id }}")'
                                                        style="color: orange;"></i>
                                                @endif
                                                {{-- <i class="fas fa-times-circle ms-3 fa-lg cpointer" wire:click='excluir("{{ $demanda->id }}")' style="color: red;"></i> --}}
                                                <i class="fas fa-check ms-3 fa-lg cpointer"
                                                    wire:click='finalizar({{ $demanda->id }})'
                                                    style="color: green;"></i>
                                                <button type="button" class="btn header-item noti-icon waves-effect"
                                                    wire:click="$emit('carregaModalComentarios', {{ $demanda->id }})">
                                                    <i class="bx bx-message-alt-dots" style="color: darkcyan"></i>
                                                    <span
                                                        class="badge bg-danger rounded-pill">{{ $demanda->comentarios->count() }}</span>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="col-3">
                ...
                <div class="col-12 mb-4"
                    style="display: flex;flex-wrap: wrap;max-width: 95%;gap: 5px;margin: 0 auto;">
                    <a name="" id="" class="btn btn-primary btn-setor   @if ($setor == 2) ativo @endif "
                        wire:click="trocaSetor(2)" style="flex: 50%;">Sistema</a>
                    <a name="" id="" class="btn btn-primary btn-setor  @if ($setor == 0) ativo @endif"
                        wire:click="trocaSetor(0)" style="flex: 50%;">Sócios</a>
                    <a name="" id="" class="btn btn-primary btn-setor  @if ($setor == 1) ativo @endif"
                        wire:click="trocaSetor(1)" style="flex: 50%;">Gerência</a>
                    <a name="" id="" class="btn btn-primary btn-setor  @if ($setor == 3) ativo @endif"
                        wire:click="trocaSetor(3)" style="flex: 50%;">Mini Contratos</a>
                </div>
                        ...
                        <form id="form-filtro" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control" name="" id="" wire:model='finalizada'>
                                    ...
                                    <option value="0" active>Em aberto</option>
...
@push('styles')
    <style>
        ...
        .accordion-button.vencida {
            background-color: #ff000086 !important;
